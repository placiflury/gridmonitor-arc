...
class User(object):
    ...
    def reset_passwd(self,pwd):
        ...
        """
        privkey = config['sft_private_key']
        new_privkey = config['sft_new_private_key']
    ...
    def get_passwd(self):
        ...
        if new_privkey:
            try:
                ...
                rc = rsa.RSACipher(new_privkey) # we do not catch except here
                plain_passwd = rc.private_decrypt(self.passwd) 
        else:
            rc = rsa.RSACipher(privkey)
            plain_passwd = rc.private_decrypt(self.passwd) 
